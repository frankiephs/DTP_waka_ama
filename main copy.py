...
class gui_component():
    ...
    def home_screen(self):
        ...
        # Settings button
        settings_button = ct.CTkButton(nav_bar_frame,text="Home",width=70,command=self.help_screen,fg_color="white",text_color="black") # maximize the button width to the text
        settings_button.grid(row=0,column=0,pady=2) # displaces the help button on the 'north east'

        # Logs button
        logs_button = ct.CTkButton(nav_bar_frame,text="Settings",width=70,command=self.help_screen,fg_color="transparent") # maximize the button width to the text
        logs_button.grid(row=0,column=1,pady=2,padx=20) # displaces the help button on the 'north east'

        # Help button
        help_button = ct.CTkButton(nav_bar_frame,text="Help",width=70,command=self.help_screen,fg_color="transparent") # maximize the button width to the text
        help_button.grid(row=0,column=2,pady=2,padx=1) # displaces the help button on the 'north east'
    ...
    def year_details_screen(self):
        ...
        # Back button
        back_button = ct.CTkButton(nav_bar_frame,text="Back",width=0)
        back_button.grid(sticky="w",padx=10,pady=10)

        # body frame
        body_frame = ct.CTkFrame(year_details_frame,fg_color="#18191B",border_width=self.BORDER_WIDTH,border_color=self.CORNER_COLOR,corner_radius=self.CORNDER_RADIUS)
        ...
        # year input
        self.year_input = ct.CTkEntry(body_frame,placeholder_text="Type year",placeholder_text_color=self.PLACEHOLDER_COLOR,border_width=self.BORDER_WIDTH,corner_radius=self.CORNDER_RADIUS,border_color=self.CORNER_COLOR,fg_color=self.BACKGROUND_COLOR) # Creates the year input box
        ...
        # keyword input
        self.keyword_input = ct.CTkEntry(body_frame,placeholder_text="Type keyword",placeholder_text_color=self.PLACEHOLDER_COLOR,border_width=self.BORDER_WIDTH,corner_radius=self.CORNDER_RADIUS,border_color=self.CORNER_COLOR,fg_color=self.BACKGROUND_COLOR) # Creates an input box for the keyword
        ...
        table = CTkTable(body_frame, row=3, column=2, values=value,border_width=self.BORDER_WIDTH,border_color=self.CORNER_COLOR,corner_radius=self.CORNDER_RADIUS,header_color="#1F2123", colors=[self.BACKGROUND_COLOR,self.BACKGROUND_COLOR],width=50)
        table.grid(row=1,column=1,rowspan=4,padx=30)
        
        # proceed
        proceed_button = ct.CTkButton(body_frame,text="Proceed",command=self.loading_screen,width=30) # Creates the proceed button
    ...
    def results_screen(self):
        ...
        # Back button
        back_button = ct.CTkButton(nav_bar_frame,text="Back",width=0,command=self.home_screen())
        back_button.grid(sticky="w",padx=10,pady=10)
        ...
        table = CTkTable(table_frame,width=190, row=70, column=3, values=value,border_width=self.BORDER_WIDTH,border_color=self.CORNER_COLOR,corner_radius=self.CORNDER_RADIUS,header_color="#1F2123", colors=[self.BACKGROUND_COLOR,self.BACKGROUND_COLOR])
        table.grid(sticky="nsew")
        table.edit_column(0,width=10)
        table.edit_column(2,width=10)
        
        # proceed
        save_to_csv = ct.CTkButton(inner_frame,text="Save to CSV",command=self.loading_screen,width=30) # Creates the proceed button
