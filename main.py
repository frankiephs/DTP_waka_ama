...
class ProgramFunctionsComponent:
    ...
    def loading_process(gui_instance):
        ...
        def process_file(index):
            ...
            if index < len(lif_files_from_year):
                ...
                gui_instance.loading_file_label.configure(
                    text=f"Processing {lif_file_name}"
                )
                lif_file_teams = gui_instance.program_functions.read_n_categorize_file(
                    lif_file_name, gui_instance
                )
                ...
                regional_associations_results = (
                    gui_instance.program_functions.get_all_scores(
                        lif_files_contents_dir, gui_instance
                    )
                )
                regional_associations_results = (
                    gui_instance.program_functions.sort_descending(
                        regional_associations_results
                    )
                )
                gui_instance.success(
                    "Program Finished",
                    message="All regional associations successfully scored",
                )
                ...
                # export
                gui_instance.regional_associations_results = (
                    regional_associations_results
                )
        ...
        if gui_instance.program_functions.check_inputs_is_valid() is True:
            ...
            if isinstance(target_year_files, list):
                lif_files_from_year = gui_instance.program_functions.find_lif_files(
                    target_year_files
                )
            ...
            gui_instance.error(
                ...
                message=gui_instance.program_functions.check_inputs_is_valid(),
            )
            gui_instance.year_details_screen()
        ...
        if lif_files:
            ...
            if self.target_keyword:
                ...
                if lif_files_with_keyword:
                    ...
                    return (
                        f"there are no lif files with the keyword {self.target_keyword}"
                    )
            else:
    ...
    def read_n_categorize_file(self, lif_file, gui_instance):
        ...
        with open(
            f"{self.parent_folder}/WakaNats{self.target_year}/{lif_file}", "r"
        ) as file:
    ...
    def get_all_scores(self, lif_files_contents_dir, gui_instance):
        ...
        for lif_file_name in lif_files_contents_dir:
            ...
            for team in teams_list:
                ...
                points_refference = gui_instance.settings_dict[
    ...
    def sort_descending(self, regional_association_results):
        ...
        """
        regional_association_results = dict(
            sorted(
                regional_association_results.items(),
    ...
    def settings_save(self, gui_instance):
        ...
        try:
            ...
            POINTS_REFFERENCE_SETTINGS = json.loads(
                data_str
            ...
            try:
                ...
                try:
                    ...
                    gui_instance.success(
                        "Saved", "successfully saved. Program restart required"
                    )
            except:
    ...
    def save_to_csv(gui_instance):
        ...
        if csv_path != False:
            gui_instance.success(
                "Saved successfully", message=f"CSV saved to {csv_path}"
            )
        else:
...
class GuiComponent:
    ...
    def __init__(self):
        ...
        # fonts
        self.FONT_BASE_CONSTANT = (
            50  # also increases the fonts size relative to the scale constant
        )
        self.title_font = ct.CTkFont(
            size=int(
                (self.FONT_BASE_CONSTANT / self.SCALE_CONSTANT) * self.SCALE_CONSTANT
            ),
            weight="bold",
        )  # Creates a bold and big font for the title
        self.heading_font = ct.CTkFont(
            size=int(self.FONT_BASE_CONSTANT / self.SCALE_CONSTANT)
            - self.FONT_BASE_CONSTANT,
            weight="bold",
        )  # Creates a bold and big font for the title
        ...
        # window scaling
        ct.set_window_scaling(
            self.SCALE_CONSTANT * 0.4
        )  # Increase the widget size by scale constant
        ct.set_widget_scaling(
            self.SCALE_CONSTANT * 0.9
        )  # Increase window size by scale constant

        # theme
        ct.set_appearance_mode(
            self.APPEARANCE_MODE
        )  # makes the UI light mode. There is dark,system,light.
        ct.set_default_color_theme(
            self.COLOR_THEME
        )  # changes the theme. There is 'blue' (standard), 'green', 'dark-blue'

        # maximize root
        self.root.rowconfigure(0, weight=1)  # maximize the root window to only 1 row
        self.root.columnconfigure(
            0, weight=1
        )  # maximize the root window to only 1 column
    ...
    def home_screen(self):
        ...
        # nav bar frame
        nav_bar_frame = ct.CTkFrame(
            homepage_frame, height=20, **self.NAV_BAR_FRAME_KWARGS
        )
        ...
        home_button = ct.CTkButton(
            nav_bar_frame, text="Home", **self.NAV_BUTTON_SELECTED
        )  # maximize the button width to the text
        home_button.grid(
            row=0, column=0, **self.NAV_BAR_PADDING
        )  # displaces the help button on the 'north east'
        ...
        settings_button.grid(
            row=0, column=1, **self.NAV_BAR_PADDING
        )  # displaces the help button on the 'north east'

        # Help button
        help_button = ct.CTkButton(
            nav_bar_frame,
            text="Help",
        ...
        help_button.grid(
            row=0, column=2, **self.NAV_BAR_PADDING
        )  # displaces the help button on the 'north east'
        ...
        # inner frame
        inner_frame = ct.CTkFrame(
            body_frame, fg_color=self.BACKGROUND_COLOR
        )  # Creates the inner frame for the input boxes, etc.
        ...
        title_label = ct.CTkLabel(
            title_group, text="Waka Ama leaderboard system", font=self.title_font
        )
        title_label.grid()
        ...
        subtitle_label = ct.CTkLabel(
            title_group,
            text="A program that scores your Waka Ama National results and saves to CSV",
        ...
        # input parent path
        self.input_parent = ct.CTkEntry(
            input_group,
            **self.INPUT_KWARGS,
            width=300,
            placeholder_text="Type your parent folder's location",
        )
        self.input_parent.grid(
            row=0,
            column=0,
            sticky="e",
            pady=self.INPUT_GROUP_PADDING,
            padx=(self.INPUT_GROUP_PADDING, 0),
        )  # displays the open folder

        # open folder button
        self.parent_folder_button = ct.CTkButton(
            input_group,
            text="Open folder",
            command=self.pick_folder,
            width=50,
            **self.OUTLINE_BUTTON_KWARGS,
        )  # Creates a button to open folder
        self.parent_folder_button.grid(
            row=0, column=1, sticky="w", padx=10, pady=self.INPUT_GROUP_PADDING
        )  # displays the open folder
        ...
        self.parent_folder_button = ct.CTkButton(
            input_group,
            text="Next",
            command=self.parent_path_submit,
            width=50,
            **self.SOLID_BUTTON_KWARGS,
        )  # Creates a button to open folder
        self.parent_folder_button.grid(
            row=0,
            column=2,
            sticky="w",
            padx=(10, self.INPUT_GROUP_PADDING),
            pady=self.INPUT_GROUP_PADDING,
        )  # displays the open folder
    ...
    def year_details_screen(self):
        ...
        heading_label = ct.CTkLabel(
            body_frame, text="Input Details", font=self.heading_font
        )
        ...
        self.year_input = ct.CTkEntry(
            ...
            border_color=self.BORDER_COLOR,
            fg_color=self.BACKGROUND_COLOR,
        )  # Creates the year input box
        self.year_input.grid(
            row=2, column=0, padx=30, pady=(0, 10), sticky="nsew"
        )  # displays the year input box
        ...
        self.keyword_input = ct.CTkEntry(
            ...
            border_color=self.BORDER_COLOR,
            fg_color=self.BACKGROUND_COLOR,
        )  # Creates an input box for the keyword
        self.keyword_input.grid(
            row=4, column=0, padx=30, sticky="nsew"
        )  # displays the keyword input box
        ...
        table = CTkTable(
            body_frame, row=1, column=2, values=value, width=50, **self.TABLE_KWARGS
        )
        ...
        proceed_button.grid(
            row=5, column=1, padx=30, pady=20, sticky="e"
        )  # displays the proceed button
    ...
    def loading_screen(self):
        ...
        self.loading_file_label = ct.CTkLabel(
            inner_frame, text=f"Processing {self.current_loading_file}"
        )  # creates the loading text message
        self.loading_file_label.grid()  # displays the loading text message

        # add the loading
        self.loading_progressbar = ct.CTkProgressBar(
            inner_frame
    ...
    def results_screen(self):
        ...
        # heading
        heading_label = ct.CTkLabel(
            inner_frame, text="Regional Association Results", font=self.title_font
        )
        ...
        subheading_label = ct.CTkLabel(
            inner_frame,
            text=f"This is the regional association results of {self.year_input.get()} {self.keyword_input.get()}",
        )
        ...
        for index, (regional_association, score) in enumerate(
            self.regional_associations_results.items()
        ):
            value.append([index + 1, regional_association, score])

        table_frame = ct.CTkScrollableFrame(
            inner_frame, height=100, width=300, fg_color=self.BACKGROUND_COLOR
        )
        ...
        # save to CSV
        save_to_csv = ct.CTkButton(
            inner_frame,
            text="Save to CSV",
            command=lambda: self.program_functions.save_to_csv(self),
            width=30,
            **self.SOLID_BUTTON_KWARGS,
        )  # Creates the proceed button
        save_to_csv.grid(
            row=3, column=0, padx=30, pady=20, sticky="e"
        )  # displays the proceed button
    ...
    def settings_screen(self):
        ...
        # nav bar frame
        nav_bar_frame = ct.CTkFrame(
            settings_screen_frame, height=20, **self.NAV_BAR_FRAME_KWARGS
        )
        nav_bar_frame.grid(row=0, column=0, padx=20, pady=20)
        nav_bar_frame.columnconfigure(0, weight=0)
        nav_bar_frame.columnconfigure(1, weight=0)
        ...
        # Settings button
        settings_button = ct.CTkButton(
            nav_bar_frame, text="Settings", **self.NAV_BUTTON_SELECTED
        )  # maximize the button width to the text
        settings_button.grid(
            row=0, column=1, **self.NAV_BAR_PADDING
        )  # displaces the help button on the 'north east'
        ...
        # body frame
        body_frame = ct.CTkScrollableFrame(
            settings_screen_frame, **self.SOLID_FRAME_KWARGS, width=400, height=400
        )
        body_frame.grid(row=1, column=0, padx=100, pady=(30, 50))
        body_frame.rowconfigure(0, weight=1)
        body_frame.columnconfigure(0, weight=1)

        save_frame = ct.CTkFrame(
            body_frame, fg_color=self.BACKGROUND_COLOR, height=50, width=50
        )
        ...
        save_button = ct.CTkButton(
            ...
            **self.SOLID_BUTTON_KWARGS,
            text="save",
            width=20,
            command=lambda: self.program_functions.settings_save(self),
        )
        save_button.grid(sticky="w", padx=10, pady=10)

        dq_frame = ct.CTkFrame(
            body_frame, fg_color=self.BACKGROUND_COLOR, height=50, width=50
        )
        ...
        self.dq_switch = ct.CTkSwitch(
            dq_frame, text="Do not include Disqualified/Did not start"
        )
        self.dq_switch.grid(row=0, column=0, sticky="w")

        show_errors_frame = ct.CTkFrame(
            body_frame, fg_color=self.BACKGROUND_COLOR, height=50, width=50
        )
        ...
        scoring_criteria_frame = ct.CTkFrame(
            body_frame, fg_color=self.BACKGROUND_COLOR, height=50, width=50
        )
        ...
        scoring_criteria_label = ct.CTkLabel(
            scoring_criteria_frame, text="Scoring Criteria"
        )
    ...
    def help_screen(self):
        ...
        # nav bar frame
        nav_bar_frame = ct.CTkFrame(
            help_screen_frame, height=20, **self.NAV_BAR_FRAME_KWARGS
        )
        nav_bar_frame.grid(row=0, column=0, padx=20, pady=20)
        nav_bar_frame.columnconfigure(0, weight=0)
        nav_bar_frame.columnconfigure(1, weight=0)
        ...
        # Help button
        help_button = ct.CTkButton(
            nav_bar_frame, text="Help", **self.NAV_BUTTON_SELECTED
        )  # maximize the button width to the text
        help_button.grid(
            row=0, column=2, **self.NAV_BAR_PADDING
        )  # displaces the help button on the 'north east'
    ...
    def pick_folder(self):
        ...
        if folder_path:  # checks if folder exists
            ...
            self.error(
                title="folder error", message="pick a folder"
            )  # open error dialog. Folder error and the messaeg 'Cannot open folder'

    def remove_current_screen(
        self,
    ):  # creates a function for removing the current screen
